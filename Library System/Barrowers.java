...
public class Barrowers extends JInternalFrame implements ActionListener 
{
	...
	public Barrowers(Connection conn, JFrame getParentFrame) throws SQLException
	{
		...
		setFrameIcon(new ImageIcon("images/barrowers.gif"));
		setSize(728,450);
	...
	ActionListener JBActionListener = new ActionListener()
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if(srcObj == "add")
			{
				JDialog JDAdd = new AddEditBarrowers(true,JFParentFrame,cnBarrowers,"");
				JDAdd.show();
			} 
			else if(srcObj == "edit")
			{
				if(total != 0){
					try
					{
						if(JTBrrwrsTbl.getValueAt(JTBrrwrsTbl.getSelectedRow(),JTBrrwrsTbl.getSelectedColumn()) != null)
						{
							JDialog JDEdit = new AddEditBarrowers(false,JFParentFrame,cnBarrowers,"SELECT * FROM tblBarrowers WHERE BarrowersID ='" + JTBrrwrsTbl.getValueAt(JTBrrwrsTbl.getSelectedRow(),0) + "'");
							JDEdit.show();
						}
					}
					catch(Exception sqlE)
			...
			{
				JDialog JDSearch = new Search(JFParentFrame, "Barrowers");
				JDSearch.show();			
			}
